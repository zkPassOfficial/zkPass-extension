...
export class Hash {
  ...
  constructor(initState?: Uint32Array) {
    if(initState){
      this._restoreState(initState, blockSize)
    }else{
      this.reset()
    }
...
export function hash(data: Uint8Array, state?: Uint32Array): Uint8Array {
  const h = (new Hash(state)).update(data)
